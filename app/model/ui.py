...
def inject_css():
    ...
    [data-testid="stMultiSelect"] > div { background: #0A1F3B !important; border-color: rgba(255,255,255,0.12) !important; }

    /* ── Success / info boxes ── */
    [data-testid="stAlert"][data-baseweb="notification"][kind="success"] { border-left-color: #27AE60 !important; }
    [data-testid="stAlert"][data-baseweb="notification"][kind="info"]    { border-left-color: #4A90D9 !important; }
    ...
    .team-name { font-size: 0.88rem; color: #8BA3C1; }
    .team-name.winner { color: #FFFFFF; font-weight: 700; }
    .prob-pill {
    ...
    .prob-pill.winner { background: rgba(240,180,41,0.12); color: #F0B429; }
    .match-vs {
    ...
    .stage-arrow {
        ...
        font-size: 1.4rem; padding: 6px 0; margin: 2px 0;
    }
    .champion-card {
    ...
    .champion-name { color: #FFFFFF; font-size: 1.4rem; font-weight: 800; margin-bottom: 4px; }
    .champion-prob { color: #C8A830; font-size: 0.88rem; font-weight: 600; }
    </style>
...
def render_footer():
    st.markdown(
        ...
        f'Built by Jadiel Montero &nbsp;&middot;&nbsp; '
        f'<a href="{LINKEDIN}" target="_blank">LinkedIn</a> &nbsp;&middot;&nbsp; '
        f'<a href="{GITHUB_USER}" target="_blank">GitHub</a>'
        f'</div>',
        unsafe_allow_html=True,
    )
...
def render_github_link():
    st.markdown(
        ...
        f'style="color:#4A7AAA; font-weight:600; font-size:0.9rem; text-decoration:none;">'
        f'View full model documentation on GitHub &rarr;</a></div>',
        unsafe_allow_html=True,
    )
...
def _invalidate_downstream(pick_key):
    ...
    if stage == "R16":
        qf_i = idx // 2
        bp.pop(f"QF_{qf_i}", None)
        sf_i = qf_i // 2
        bp.pop(f"SF_{sf_i}", None)
        bp.pop("Final_0", None)
    elif stage == "QF":
        sf_i = idx // 2
        bp.pop(f"SF_{sf_i}", None)
...
def _fill_defaults(model, team_elos):
    ...
    for i, (a, b) in enumerate(r16):
        bp.setdefault(f"R16_{i}", _default_winner(model, team_elos, a, b))

    for i in range(4):
        qa = bp.get(f"R16_{2*i}"); qb = bp.get(f"R16_{2*i+1}")
        if qa and qb:
            bp.setdefault(f"QF_{i}", _default_winner(model, team_elos, qa, qb))

    for i in range(2):
        sa = bp.get(f"QF_{2*i}"); sb = bp.get(f"QF_{2*i+1}")
        if sa and sb:
            bp.setdefault(f"SF_{i}", _default_winner(model, team_elos, sa, sb))

    fa = bp.get("SF_0"); fb = bp.get("SF_1")
    if fa and fb:
...
def _match_card(team_a, team_b, pick_key, model, team_elos):
    ...
    def _row(name, prob, is_winner):
        hl = "border-left:2px solid #F0B429; padding-left:7px; margin-left:-7px;" if is_winner else ""
        tc = "#FFFFFF" if is_winner else "#8BA3C1"
        fw = "700" if is_winner else "400"
        pc_bg = "rgba(240,180,41,0.12)" if is_winner else "rgba(255,255,255,0.05)"
        pc_c  = "#F0B429" if is_winner else "#445566"
        return (
            f'<div style="display:flex;justify-content:space-between;align-items:center;'
            f'padding:5px 0;{hl}">'
            f'<span style="color:{tc};font-weight:{fw};font-size:0.87rem;">{name}</span>'
            f'<span style="background:{pc_bg};color:{pc_c};border-radius:4px;'
            f'padding:2px 7px;font-size:0.73rem;font-weight:700;">{prob:.0%}</span>'
            f'</div>'
        )
...
def render_interactive_bracket(matchup_counts, model, team_elos, N):
    ...
    for i, (a, b) in enumerate(r16):
        with cols[i % 4]:
            _match_card(a, b, f"R16_{i}", model, team_elos)
    ...
    cols = st.columns(4)
    for i in range(4):
        qa = bp.get(f"R16_{2*i}", "TBD")
        qb = bp.get(f"R16_{2*i+1}", "TBD")
        with cols[i]:
            if qa != "TBD" and qb != "TBD":
                _match_card(qa, qb, f"QF_{i}", model, team_elos)
    ...
    for i, col in enumerate([c1, c2]):
        sa = bp.get(f"QF_{2*i}", "TBD")
        sb = bp.get(f"QF_{2*i+1}", "TBD")
        with col:
            if sa != "TBD" and sb != "TBD":
                _match_card(sa, sb, f"SF_{i}", model, team_elos)
    ...
    if champion and fa != "TBD" and fb != "TBD":
        ...
        _, col, _ = st.columns([2, 3, 2])
        with col:
            p_win = _win_prob_between(
                model, team_elos,
                champion,
                fb if champion == fa else fa,
            )
            st.markdown(
                f'<div class="champion-card">'
                f'<div class="champion-label">Predicted Champion</div>'
                f'<div class="champion-name">{champion}</div>'
                f'<div class="champion-prob">{p_win:.0%} win probability in final</div>'
                f'</div>',
                unsafe_allow_html=True,
            )
